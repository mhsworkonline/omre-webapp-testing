/**
 * merge-results.js
 * Merges a partial re-run JSON into the original full-run JSON, then writes Excel.
 *
 * Base  = JSON with the MOST tests (= full run).
 * Patch = most recent JSON (= partial re-run).
 * Only overwrites tests that actually ran in the patch (non-empty results).
 *
 * Usage:
 *   node merge-results.js                          # auto-detect base + patch
 *   node merge-results.js base.json patch.json     # explicit filenames
 */
const fs      = require('fs');
const path    = require('path');
const ExcelJS = require('exceljs');

const resultsDir = path.join(__dirname, 'test-results');
const jsonFiles  = fs.readdirSync(resultsDir)
  .filter(f => f.endsWith('.json') && /^\d{4}-\d{2}-\d{2}/.test(f))
  .sort();

if (jsonFiles.length < 2) {
  console.error('Need at least 2 JSON result files.');
  process.exit(1);
}

// ── Count total tests in a parsed JSON ───────────────────────────────────────
function countTests(data) {
  let n = 0;
  function walk(suites) {
    for (const s of suites || []) {
      for (const spec of s.specs || []) n += (spec.tests || []).length;
      walk(s.suites || []);
    }
  }
  walk(data.suites || []);
  return n;
}

// ── Resolve base and patch ────────────────────────────────────────────────────
let baseFile, patchFile;

if (process.argv[2] && process.argv[3]) {
  baseFile  = process.argv[2];
  patchFile = process.argv[3];
} else {
  // Patch = most recent JSON. Base = most recent JSON older than patch with most tests.
  const sorted  = [...jsonFiles].sort((a, b) => b.localeCompare(a)); // newest first
  patchFile     = sorted[0];
  const candidates = sorted.slice(1).map(f => {
    const data = JSON.parse(fs.readFileSync(path.join(resultsDir, f), 'utf8'));
    return { f, count: countTests(data) };
  });
  baseFile = candidates.sort((a, b) => b.count - a.count)[0].f;
}

console.log('Base  (full run):', baseFile);
console.log('Patch (re-run):  ', patchFile);

const base  = JSON.parse(fs.readFileSync(path.join(resultsDir, baseFile),  'utf8'));
const patch = JSON.parse(fs.readFileSync(path.join(resultsDir, patchFile), 'utf8'));

// ── Index patch tests — only those that actually ran (have results) ───────────
const patchIndex = {};

function tcId(title) {
  const m = title.match(/TC-[\w]+-[\w]+/i);
  return m ? m[0].toUpperCase() : title;
}

function indexSuites(suites) {
  for (const suite of suites || []) {
    for (const spec of suite.specs || []) {
      const hasResults = (spec.tests || []).some(t => (t.results || []).length > 0);
      if (hasResults) {
        patchIndex[tcId(spec.title)] = spec.tests;
      }
    }
    indexSuites(suite.suites || []);
  }
}
indexSuites(patch.suites || []);

// ── Patch base JSON ───────────────────────────────────────────────────────────
let replaced = 0;

function patchSuites(suites) {
  for (const suite of suites || []) {
    for (const spec of suite.specs || []) {
      const key = tcId(spec.title);
      if (patchIndex[key]) {
        spec.tests = patchIndex[key];
        replaced++;
      }
    }
    patchSuites(suite.suites || []);
  }
}
patchSuites(base.suites || []);
console.log(`Merged ${replaced} test(s).`);

// ── Flatten ───────────────────────────────────────────────────────────────────
function moduleFromFile(file) {
  const f = file.replace(/\\/g, '/');
  const map = {
    'home/home':               'Home',
    'home/post-creation':      'Home',
    'home/stories':            'Home',
    'home/interactions':       'Home',
    'explore/explore':         'Explore',
    'explore/shorts':          'Shorts',
    'content/live':            'Live',
    'social/messages':         'Messages / Chat',
    'social/notifications':    'Notifications',
    'profile/profile':         'Profile',
    'profile/reputation':      'Reputation',
    'platform/business-suite': 'Business Suite',
    'features/settings':       'Settings',
    'platform/omniknow':       'OmniKnow',
    'platform/happy-corner':   'Happy Corner',
    'platform/virtual-world':  'Virtual World',
    'platform/digital-citizen':'Digital Citizen',
    'platform/omni-ai':        'Omni AI',
    'social/pages':            'Pages',
    'social/groups':           'Groups',
    'platform/town-hall':      'Town Hall',
    'content/birthday':        'Birthday',
    'content/weather':         'Weather',
    'content/images':          'Images',
    'social/friends':          'Friends',
    'content/news':            'News',
    'features/video':          'Video',
    'platform/biz':            'Biz',
    'platform/link':           'Link (Jobs)',
    'platform/learn':          'Learn',
    'features/studio':         'Studio',
    'platform/orbit':          'Orbit',
    'features/games':          'Games',
    'features/mart':           'Mart',
    'features/meetings':       'Meetings',
    'features/wallet':         'Wallet',
    'features/ai-studio':      'AI Studio',
    'channel/my-channel':      'Channel',
    'channel/subscriptions':   'Subscriptions',
    'social/posts':            'Posts',
    'auth/login':              'Auth',
    'errors/error-states':     'Error States',
    'responsive/mobile':       'Mobile / Responsive',
    'navigation/cross-module': 'Navigation',
    'performance/performance': 'Performance',
    'security/security':       'Security',
    'features/theme':          'Theme',
  };
  for (const [key, val] of Object.entries(map)) {
    if (f.includes(key)) return val;
  }
  const parts = f.split('/tests/')[1]?.replace('.spec.js','').split('/') || [];
  return parts.map(p => p.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())).join(' › ');
}

function flatten(suites, file) {
  const rows = [];
  for (const suite of suites || []) {
    const f = file || suite.file || '';
    for (const spec of suite.specs || []) {
      for (const test of spec.tests || []) {
        const result   = test.results?.[0] || {};
        const status   = result.status === 'passed'  ? 'PASS'
                       : result.status === 'failed'  ? 'FAIL'
                       : result.status === 'skipped' ? 'SKIP'
                       : result.status?.toUpperCase() || 'UNKNOWN';
        const duration = result.duration ? `${(result.duration / 1000).toFixed(1)}s` : '-';
        const title    = spec.title || test.title || '';
        const idMatch  = title.match(/TC-[\w-]+/);
        const errorMsg = result.errors?.[0]?.message || result.error?.message || '';
        const remarks  = status === 'FAIL' ? errorMsg.split('\n')[0].replace(/\s+/g, ' ').trim().slice(0, 200)
                       : status === 'SKIP' ? 'Optional UI not present on this account'
                       : '';
        rows.push({
          module: moduleFromFile(f),
          id:     idMatch ? idMatch[0] : '-',
          name:   title.replace(/^TC-[\w-]+[:\s]+/i, '').trim(),
          status, duration, remarks,
        });
      }
    }
    rows.push(...flatten(suite.suites || [], f));
  }
  return rows;
}

const rows    = flatten(base.suites || []);
const grouped = {};
for (const row of rows) {
  if (!grouped[row.module]) grouped[row.module] = [];
  grouped[row.module].push(row);
}

// ── Excel ─────────────────────────────────────────────────────────────────────
const wb = new ExcelJS.Workbook();
wb.creator = 'OMRE QA Suite';
wb.created = new Date();

const summary = wb.addWorksheet('Summary');
summary.columns = [
  { header: 'Module', key: 'module', width: 28 },
  { header: 'Total',  key: 'total',  width: 8  },
  { header: 'PASS',   key: 'pass',   width: 8  },
  { header: 'FAIL',   key: 'fail',   width: 8  },
  { header: 'SKIP',   key: 'skip',   width: 8  },
];
summary.getRow(1).font = { bold: true };
for (const [mod, tests] of Object.entries(grouped).sort()) {
  summary.addRow({
    module: mod,
    total:  tests.length,
    pass:   tests.filter(t => t.status === 'PASS').length,
    fail:   tests.filter(t => t.status === 'FAIL').length,
    skip:   tests.filter(t => t.status === 'SKIP').length,
  });
}
summary.addRow({});
const totals = summary.addRow({
  module: 'TOTAL',
  total:  rows.length,
  pass:   rows.filter(t => t.status === 'PASS').length,
  fail:   rows.filter(t => t.status === 'FAIL').length,
  skip:   rows.filter(t => t.status === 'SKIP').length,
});
totals.font = { bold: true };

const details = wb.addWorksheet('All Tests');
details.columns = [
  { header: 'Module',    key: 'module',   width: 28 },
  { header: 'Test ID',   key: 'id',       width: 18 },
  { header: 'Test Name', key: 'name',     width: 60 },
  { header: 'Status',    key: 'status',   width: 10 },
  { header: 'Duration',  key: 'duration', width: 10 },
  { header: 'Remarks',   key: 'remarks',  width: 60 },
];
details.getRow(1).font = { bold: true };
for (const [, tests] of Object.entries(grouped).sort()) {
  for (const t of tests) details.addRow(t);
}

const ts      = baseFile.replace('.json', '');
const outFile = path.join(resultsDir, `${ts}_merged.xlsx`);
wb.xlsx.writeFile(outFile).then(() => {
  console.log('Merged Excel saved:', outFile);
});
